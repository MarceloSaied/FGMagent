...
Func _AD_HasFullRights($sObject, $sUser = @UserName)
	...
	Local $iResult = _AD_HasRequiredRights($sObject, $ADS_FULL_RIGHTS, $sUser = @UserName)
...
#cs ; Neue aktualisierte Version. http://www.autoitscript.com/forum/topic/146785-active-directory-udf-help-support-ii/page__st__80#entry1051664
	Func _AD_HasRequiredRights($sObject, $iRight = $ADS_FULL_RIGHTS, $sUser = @UserName, $aMemberOf = "")

	If _AD_ObjectExists($sUser) = 0 Then Return SetError(1, 0, 0)
	If _AD_ObjectExists($sObject) = 0 Then Return SetError(2, 0, 0)
	If StringMid($sObject, 3, 1) <> "=" Then $sObject = _AD_SamAccountNameToFQDN($sObject) ; sAMAccountName provided
	Local $aTrusteeArray, $sTrusteeGroup, $sTrusteeArrayFQDN
	If $aMemberOf  = "" Then $aMemberOf = _AD_GetUserGroups($sUser, 1)
	Local $oObject = __AD_ObjGet("LDAP://" & $sAD_HostServer & "/" & $sObject)
	If IsObj($oObject) Then
	Local $oSecurity = $oObject.Get("ntSecurityDescriptor")
	Local $oDACL = $oSecurity.DiscretionaryAcl
	...
	$aTrusteeArray = StringSplit($oACE.Trustee, "\")
	$sTrusteeGroup = $aTrusteeArray[$aTrusteeArray[0]]
	If (UBound($aTrusteeArray) - 1) = 2 And $aTrusteeArray[2] = $sUser And BitAND($oACE.AccessMask, $iRight) = $iRight Then Return 1
	If (UBound($aTrusteeArray) - 1) = 2 Then
	$sTrusteeArrayFQDN = _AD_SamAccountNameToFQDN($aTrusteeArray[2])
	EndIf
	For $iCount1 = 0 To UBound($aMemberOf) - 1
	If StringInStr($aMemberOf[$iCount1], $sTrusteeArrayFQDN) And _
	BitAND($oACE.AccessMask, $iRight) = $iRight Then Return 1
	If StringInStr($aMemberOf[$iCount1], "CN=" & $sTrusteeGroup & ",") And _
	BitAND($oACE.AccessMask, $iRight) = $iRight Then Return 1
	Next
...
Func _AD_HasGroupUpdateRights($sObject, $sUser = @UserName)
	...
	Local $iResult = _AD_HasRequiredRights($sObject, $ADS_OBJECT_READWRITE_ALL, $sUser = @UserName)
	Return SetError(@error, @extended, $iResult)
...
Func _AD_GetGroupMembers($sGroup)
	...
	While 1
		...
		If $aMembersadd = 0 Then ExitLoop
...
Func _AD_SetADOProperties($sP1 = "", $sP2 = "", $sP3 = "", $sP4 = "", $sP5 = "", $sP6 = "", $sP7 = "", $sP8 = "", $sP9 = "", $sP10 = "")
	...
	For $iIndex = 0 To UBound($aProperties) - 1
		...
		If @error <> 0 Then Return SetError(@error, $iIndex, 0)
	Next
	Return 1
...
Func _AD_VersionInfo()
	...
	Local $aOL_VersionInfo[9] = [8, "V", 1, 4, 0.0, "20140103", "3.3.6.0", "Jonathan Clelland, water", _
